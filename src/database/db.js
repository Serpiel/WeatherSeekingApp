...
// 1. On crée une variable globale pour garder la connexion active
let dbInstance = null;

// 2. On modifie notre fonction pour réutiliser la connexion si elle existe déjà
const getDb = async () => {
  if (!dbInstance) {
...
export const initDB = async () => {
  try {
    ...
    // On peut regrouper les requêtes de création
    await db.execAsync(`
...
export const addSearchToHistory = async (cityName) => {
  try {
    const db = await getDb();
    // 3. On encapsule la variable dans un tableau [cityName]
